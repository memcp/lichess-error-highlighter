...
def games():
    ...
    for group in win_opening_groups:
        ...
        try:
            winrate = str((number_of_wins / number_of_games) * 100) + '%'
        except:
            winrate = "-"

        openening_stats[str(group['short_name'])] = {
            'wins': number_of_wins,
            'games': number_of_games,
            'winrate': winrate
        }

    return openening_stats


@app.route('/upload-games/<token>')
def load_games(token):
    ...
    for game in games:
        ...
        insert_game(conn, game_id, game_is_lost, player_id, opening_id)
